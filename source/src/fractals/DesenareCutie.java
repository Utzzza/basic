...
/**
 * 
 *  Aceasta clasa fixeaza punctul de start sau chenarele pe imaginea incarcata 
 *  in panoul acesteia.
 */
public class DesenareCutie
{
	...
	public DesenareCutie(SetarePunct pS, BufferedImage bI, ImagePanel iP)
	{
		pSet = pS;
		bImage = bI;
		iPanel = iP;
	...
	public void drawBoxes(Pozitionare placement, double boxDim, double boxSize)
	{
		int wImg = bImage.getWidth();
		int hImg = bImage.getHeight();
		SetarePunct pRot = pSet.rotatePointSet(placement.angle);
		Punct[] rPoints = pRot.getPoints();
		double sinA = Math.sin(-placement.angle);
		double cosA = Math.cos(-placement.angle);
		Dreptunghi rBounds = pSet.determineBounds();
		...
		for (int p = 0; p < pRot.getNumPoints(); p++)
		{
			...
			if (empty[r][c])
			{
				...
				DecupareLinie l = new DecupareLinie(x1, y1, x2, y2, 0, 0, wImg - 1, hImg - 1);
				if (l.isClippable())
				{
					gFractal.setColor(Color.yellow);
					gFractal.drawLine((int)l.x1, (int)l.y1, (int)l.x2, (int)l.y2);
				}
				l = new DecupareLinie(x2, y2, x3, y3, 0, 0, wImg - 1, hImg - 1);
				if (l.isClippable())
				{
					gFractal.setColor(Color.yellow);
					gFractal.drawLine((int)l.x1, (int)l.y1, (int)l.x2, (int)l.y2);
				}
				l = new DecupareLinie(x3, y3, x4, y4, 0, 0, wImg - 1, hImg - 1);
				if (l.isClippable())
				{
					gFractal.setColor(Color.yellow);
					gFractal.drawLine((int)l.x1, (int)l.y1, (int)l.x2, (int)l.y2);
				}
				l = new DecupareLinie(x4, y4, x1, y1, 0, 0, wImg - 1, hImg - 1);
				if (l.isClippable())
				{
					gFractal.setColor(Color.yellow);
					gFractal.drawLine((int)l.x1, (int)l.y1, (int)l.x2, (int)l.y2);
	...
	public void drawPoints()
	{
		...
		Dreptunghi bounds = pSet.determineBounds();
